zones = {
    'R1' : [
        'Kilimani', 'Kileleshwa', 'Riverside Drive','Spring Valley','Thompson','Woodley','Upper Spring Valley', 'Kyuna', 'Loresho', # Dagoret North + Kabete?
        'Westlands', 'Lavington', 'Muthaiga',    # Others
        'Karen',
        'Gigiri', 'Kitisuru', 'Ridgeways', 'Garden Estate', 'Safari Park', 'Roysambu', 'Thome', 'Marurui',  'Mountain View'
    ],
    
    'R2' : [
        'Parklands', 'Otiende', 'Wilson', 'Nairobi West', 'Kahawa West', 'Imara Daima', 'Upper Hill', 
        'Makadara', 'Doonholm', 'Uhuru', 'Buruburu', 'Umoja', 'Umoja Innercore',    # Makadara
        'Nairobi West', 'Madaraka', 'South B', 'South C', 'Nairobi Dam', 'Ngumo', 'Highview', 'Magiwa', 'Golf Course', # LA
        'Southlands', 'Otiende', 'Ngei 1 & 2', 'Onyonka', 'Masai', 'Uhuru Gardens', 'Jonathan Ngeno', # LA Estates
        'Villa France',  # Imara
        'Nairobi South', 'Nairobi Central',
            'Pangani', 'Hospital', 'Nairobi Central', 'Ngara',  # Starehe
            'Landimawe',  # Starehe
            'California', 'Pumwani', 'Airbase',  # Kamukunji
            'Makongeni', 'Maringo/Hamza',  # Makadara

    ],

    'R3' : ['CBD', 'Kas', 'Karura', 'Fedha', 'Pipeline', 'Jogoo', 'Ofafa', 'Eastleigh', 'Ziwani','Starehe', 'Old Eastlands', 
        'Imara Daima', 'Tassia', 'Fedha', 'Avenue'
        'Kayole', 'Komarock',  # Others
        'Dagoretti', 'Riruta', 'Kangemi', 'Mutuini', 'Waithaka', 'Ruthimitu', 'Uthiru',
        'Ruaraka', 'Kahawa West',     # Baba Dogo
        'Kasarani',  #Kasarani     
           'Mwiki', 'Clay City', 'Njiru', 'Ruai',  # Kasarani
        'Lucky Summer', 'Babandogo', 'Utalii',  # Ruaraka
        'Utawala', 'Mihango', 'Lower Savanna',  # Embakasi East
        'Harambee',  # Makadara
        'Kayole North', 'Kayole Central', 'Kayole South', 'Upper Savanna', 'Mowlem',  # Embakasi Central
        'Githurai', 'Kahawa',  # Roysambu
        'Kabiro', 'Mutu-ini', 'Ngando',  # Dagoretti South
        'Mugumo-ini', 'Nyayo Highrise',  # Lang'ata
    ],

    'R4' : [
        'Dandora', 'Mathare Valley', 'Mathare North', 'Lower Huruma','Kariobangi', 
        'Kibera',   'Ayany', 'Olympic', 'Fort Jesus', 'Karanja Road', 
        'Githurai 44 & 45', 'Zimmerman', # Baba Dogo
           'Korogocho',  # Ruaraka
        'Viwandani',  # Makadara
        'Kawangware', 'Gatina',  # Dagoretti North
        'Huruma', 'Mabatini', 'Ngei', 'Mlango Kubwa', 'Kiamaiko',  # Mathare
        'Laini Saba', 'Lindi', 'Makina', "Sarang'ombe",  # Kibra
        'Kwa Njenga', 'Kwa Reuben', 'Kware',  # Embakasi South
    ]
}